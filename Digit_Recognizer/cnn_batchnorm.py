...
def add_conv2d_layer(model,filters, kernel_size,padding = 'Same', activation='relu',use_batch_norm=False, **kwargs):
        if use_batch_norm:
            model.add(Conv2D(filters, kernel_size,padding = 'Same', use_bias=False, **kwargs))
            model.add(BatchNormalization())
            model.add(Activation(activation))
        else:
            model.add(Conv2D(filters, kernel_size, activation=activation, **kwargs))
# 定义模型
def cnnModel(X_train):
    ...
    add_conv2d_layer(model = model,filters = 32, kernel_size = (5,5),padding = 'Same', 
                    activation ='relu',use_batch_norm=True, input_shape = (28,28,1))
    # model.add(Conv2D(filters = 32, kernel_size = (5,5),padding = 'Same', 
    #                 activation ='relu'))
    add_conv2d_layer(model = model,filters = 32, kernel_size = (5,5),padding = 'Same', 
    ...
    #                 activation ='relu'))
    add_conv2d_layer(model = model,filters = 64, kernel_size = (3,3),padding = 'Same', 
                    activation ='relu',use_batch_norm=True)
    # model.add(Conv2D(filters = 64, kernel_size = (3,3),padding = 'Same', 
    #                 activation ='relu'))
    add_conv2d_layer(model = model,filters = 64, kernel_size = (3,3),padding = 'Same', 
